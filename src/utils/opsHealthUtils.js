...
export function buildOpsHealth(room, gameContent = {}) {
  ...
  const teamRows = teams.map(team => {
    ...
    const evidenceReviewCount = playableRounds.filter(roundId => room.submissions?.[`${roundId}_${team.teamId}`]?.evidenceReview).length;
    const calculationCount = playableRounds.filter(roundId => room.roundCalculations?.[`${roundId}_${team.teamId}`]).length;
    const resultNarrativeCount = playableRounds.filter(roundId => room.roundCalculations?.[`${roundId}_${team.teamId}`]?.teamResultNarrative).length;
    ...
    if (submissionCount > 0 && evidenceReviewCount < submissionCount) issues.push(makeIssue({ severity: '주의', teamName: team.teamName, area: '산출물 증거 수준', message: `증거 수준 리뷰 ${evidenceReviewCount}/${submissionCount}건`, action: '산출물을 다시 저장하거나 리허설 샘플을 재생성하세요.' }));
    ...
    return {
      ...
      reflectionCount,
      status: completeKsa && expertiseLensReady && teamPlayers.length === profileList.length && calculationCount === playableRounds.length && evidenceReviewCount === submissionCount && resultNarrativeCount === calculationCount && finalReady ? '정상' : '확인 필요'
    };
  });
