...
export default function FirebaseHostStatusPage() {
  ...
  const [status, setStatus] = useState({ room: null, teams: {}, teamMembers: {}, teamDecisions: {}, submissions: {}, declarations: {}, finalResults: {}, roundCalculations: {} });
  ...
  const rows = teams.map(team => {
    ...
    const members = Object.values(status.teamMembers || {}).filter(member => member.teamId === team.teamId).sort((a, b) => Number(b.browserLastSeenAt || 0) - Number(a.browserLastSeenAt || 0));
    return {
      teamId: team.teamId,
      teamName: team.teamName || firebaseTeam.teamName || team.teamId,
      pinIssued: Boolean(firebaseTeam.representativePin),
      representativeName: firebaseTeam.representativeName || members.find(member => member.role === 'representative')?.memberName || '',
      memberCount: members.length,
      memberNames: members.map(member => `${member.memberName}${member.role === 'representative' ? '(대표)' : ''}`).join(', '),
  ...
  return (
    <Layout roomId={roomId}>
      ...
      <section className="card">
        ...
        <table>
          <thead>
            <tr><th>팀</th><th>접속</th><th>접속자</th><th>대표 PIN</th><th>대표</th><th>KSA</th><th>팀 결정</th><th>산출물</th><th>결과</th><th>선언문</th><th>최종 판정</th><th>팀 화면</th></tr>
          </thead>
          <tbody>
            {rows.map(row => (
              <tr key={row.teamId}>
                <td>{row.teamName}</td>
                <td>{row.memberCount}명</td>
                <td>{row.memberNames || '미등록'}</td>
      ...
      <section className="card">
        ...
        <div className="reportInsight">
          <p>팀원 접속 수는 Firebase 팀 화면에서 이름을 등록한 인원입니다. PIN 발급은 팀대표 입력 권한을 준비했다는 뜻입니다. KSA, 팀 결정, 산출물은 Firebase 팀 화면에서 대표가 저장해야 완료로 표시됩니다.</p>
        </div>
      </section>
    </Layout>
  );
