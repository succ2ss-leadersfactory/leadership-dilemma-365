...
function getTeamProfileSummary(profiles = {}) {
  ...
  if (!list.length) return { count: 0, average: '-', topStrengths: '-' };
  ...
  return { count: list.length, average, topStrengths };
}
  ...
  return (
    <Layout roomId={roomId}>
      <section className="card hero">
        ...
        <h2>팀원 초기 역량 프로필</h2>
        <p>Round 0에서 팀이 선택한 KSA 9개를 기준으로 팀원별 초기 수준을 자동 등록한 화면입니다.</p>
      ...
      <section className="card">
        <h3>팀별 요약</h3>
        <table>
          <thead><tr><th>팀</th><th>프로필 수</th><th>평균 수준</th><th>많이 나타난 강점</th><th>팀 화면</th></tr></thead>
          <tbody>
            {teams.map(team => {
