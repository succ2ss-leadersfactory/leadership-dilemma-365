...
export default function FirebaseTeamPage() {
  ...
  const currentPhase = remote.room?.roomProgress?.currentPhase || 'teamRoom';
  ...
  useEffect(() => {
    ...
    return subscribeFirebaseTeamScreen({
      ...
      onChange: data => {
        setRemote(data);
        setMsg('팀 참가 화면이 연결되었습니다. 대표가 저장하면 같은 팀 화면에 반영됩니다.');
      },
      onError: error => setMsg(`팀 화면 연결 확인 필요: ${error.message}`)
    });
  ...
  async function saveKsa(ksa) {
    ...
    try {
      ...
      setMsg('KSA가 저장되었습니다. 같은 팀 화면에도 반영됩니다.');
    } catch (error) {
  ...
  async function saveDecision() {
    ...
    try {
      ...
      setMsg('팀 최종 선택과 토의 요약이 저장되었습니다.');
    } catch (error) {
  ...
  async function saveSubmission(answers) {
    ...
    try {
      ...
      setMsg('산출물이 저장되었습니다. 같은 팀 화면에도 반영됩니다.');
    } catch (error) {
  ...
  if (!canUseFirebase) {
    return <Layout roomId={roomId}><section className="card hero"><p className="eyebrow">TEAM ROOM</p><h2>온라인 연결 설정 확인 필요</h2><p>강사에게 온라인 운영 준비 상태를 확인해 달라고 요청하세요.</p></section></Layout>;
  }
  ...
  return (
    <Layout roomId={roomId}>
      <section className="card hero">
        ...
        <h2>{teamName} · 팀 참가 화면</h2>
        <p>같은 팀원이 이 주소로 함께 들어오면 대표가 저장한 내용이 모든 팀원 화면에 함께 반영됩니다.</p>
        ...
        {!remote.room && <StatusNoticeCard title="팀 화면 준비 중">강사가 온라인 운영 준비를 완료하면 팀 화면이 연결됩니다. 잠시 후 다시 확인해 주세요.</StatusNoticeCard>}
      </section>
